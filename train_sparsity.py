...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    # 剪枝操作
    for i, name in enumerate(cfg_model.module_defs):
        print(f'{i}:{name}')
    if opt.prune == 1:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            if save:
                ...
                if epoch + 5 > epochs:
                    ckptname = 'AP:' + str(fi) + '_eponch:' + str(epoch) + '.pt'
                    ckptpath = wdir / ckptname
    ...
    if rank in [-1, 0]:
        # Strip optimizers
        weightslist = os.listdir(wdir)
        final = best if best.exists() else last  # final model
        for f in weightslist:
            if f.exists():
                if f.endswith('.pt'):
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', type=str, default=r'/root/hy_work/yolov5-v6.0/runs/train/billboard/weights/best.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default=r'models/billboard_s.yaml', help='model.yaml path')
    parser.add_argument('--data', type=str, default=r'data/billboard.yaml', help='data.yaml path')
